...
import {
  ...
  setPointerBindingAxis,
  setPointerBindingEase,
  setPointerBindingRange,
  setPointerBindingTarget,
} from "../store/pointerCommands";
import { DragNumber, Dropdown, type MenuItem } from "./controls";

const EASE_OPTIONS: Easing[] = ["linear", "easeIn", "easeOut", "easeInOut"];
const AXIS_LABEL: Record<PointerAxis, string> = { x: "Pointer X", y: "Pointer Y" };
...
// pointer-scrub bindings UI — one axis of the damped cursor position drives a
// clip playhead or base→state blend. see docs/specs/16-pointer-parallax.md
export function PointerBindingList() {
  const bindings = useDoc((s) => s.doc?.pointerBindings);
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {bindings.length === 0 && (
        <div className="text-[11px] text-muted-foreground/70">
          No pointer-scrubbed animations or states yet.
        </div>
      )}
      <Button
        variant="secondary"
  ...
  const targetTypeItems: MenuItem[] = [
    {
      ...
      onSelect: () =>
        ...
        setPointerBindingTarget(binding.id, {
          type: "animation",
  ...
  return (
    <div className="flex flex-col gap-1 rounded border border-border bg-muted/30 p-1.5">
      <div className="flex items-center gap-1">
        <Dropdown
          button={<>{AXIS_LABEL[binding.axis]}</>}
          items={(["x", "y"] as const).map((axis) => ({
            label: AXIS_LABEL[axis],
            checked: binding.axis === axis,
            onSelect: () => setPointerBindingAxis(binding.id, axis),
          }))}
        />
        <Dropdown
          button={<>{target.type === "animation" ? "Animation" : "State"}</>}
          items={targetTypeItems}
        />
        <span className="flex-1" />
        <button
          type="button"
          title="Delete pointer binding"
          className="text-muted-foreground transition-colors hover:text-destructive"
          onClick={() => removePointerBinding(binding.id)}
        >
          ...
          {clipList.length === 0 ? (
            ...
            <Dropdown
              ...
              items={clipList.map((c) => ({
                ...
                onSelect: () =>
                  setPointerBindingTarget(binding.id, { type: "animation", animationId: c.id }),
              }))}
            />
          )}
        </ParamRow>
          ...
          <DragNumber
            ...
            onCommit={(v, merge) =>
              setPointerBindingRange(
                binding.id,
                { start: v },
                merge ? { mergeKey: `pb:${binding.id}:start` } : undefined,
              )
            }
          />
          ...
          <DragNumber
            ...
            onCommit={(v, merge) =>
              setPointerBindingRange(
                binding.id,
                { end: v },
                merge ? { mergeKey: `pb:${binding.id}:end` } : undefined,
              )
            }
          />
        </div>
      ...
      <ParamRow label="Ease">
        <Dropdown
          ...
          items={EASE_OPTIONS.map((e) => ({
            ...
            onSelect: () => setPointerBindingEase(binding.id, e),
          }))}
        />
      </ParamRow>
    </div>
