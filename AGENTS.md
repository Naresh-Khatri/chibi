...
- M13 (auth & cloud) — **foundation only** so far: an *optional*, never-enforced auth addon (Better Auth + Drizzle + Postgres via `docker-compose.yml` on port 5487; email/password + GitHub/Google). All auth/db code is under `src/lib/**`, `src/components/auth/**`, `src/app/api/auth/**` — the runtime import boundary and scene format are untouched, and the app runs fully with no DB. Cloud save + scene hosting (+ R2/S3 for assets) are deliberately deferred; see `docs/specs/14-auth-cloud.md`.
- M14 (editable mesh) implemented — subdivision-surface control-cage modeling, reversing the PRD §9 "mesh/vertex editing" non-goal. `MeshNode.geometry` is now a discriminated union (`parametric` | `editableMesh`, no migration needed — old docs match the parametric arm as-is); pure geometry lives in `packages/runtime/src/mesh/` (topology/Catmull-Clark/triangulate/primitives/ops, no editor imports); convert-to-cage, the Subdivision Modifier + Increase Base Subdivision bake, and the full vertex/edge/face mesh-edit sub-mode (cage overlay, proxy-gizmo live preview, extrude/delete faces) live in `src/editor/store/meshCommands.ts` + `src/editor/viewport/{CageOverlay,MeshEditGizmo}.tsx`. AI can generate cages (copilot tools `add_editable_mesh`/`convert_to_editable_mesh`/`set_subdivisions`; both system prompts in `src/editor/ai/prompts.ts` teach cage-authoring rules) but per-element editing stays editor-only; see `docs/specs/15-editable-mesh.md`.
