...
- Post-M8 "clay look" upgrade: capsule primitive + `fillet` param on cylinder/cone (lathe-based edge rounding), `MeshPhysicalMaterial` with clearcoat/sheen fields, environment `exposure`/`softShadows` (PCSS)/`contactShadows` and the built-in `"soft"` studio preset (Lightformer rig, offline-safe) — shared components in `packages/runtime/src/react/EnvironmentExtras.tsx`; showcase scene `public/scenes/park.chibi.json` (File → Open); decisions recorded in specs 02/03 implementation notes.
